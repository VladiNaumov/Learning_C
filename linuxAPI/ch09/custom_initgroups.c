...
Функция `initgroups()` устанавливает все вспомогательные группы для пользователя, основываясь на данных о пользователе и его основной группе. Она также добавляет пользователя ко всем группам, в которых он состоит. Для реализации такой функции с использованием `setgroups()` и стандартных библиотек нам нужно:
