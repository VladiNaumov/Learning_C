#include <stdio.h>
#include <stdlib.h>
#include <string.h>

// Определение структуры Person
typedef struct {
    char *name;
    int age;
} Person;

// Определение структуры Book
typedef struct {
    char *title;
    char *author;
    int pages;
} Book;

// Определение структуры Car
typedef struct {
    char *brand;
    char *model;
    int year;
} Car;

// Перечисление типов
typedef enum {
    PERSON,
    BOOK,
    CAR
} ItemType;

// Определение объединения для хранения различных типов данных
typedef union {
    Person *person;
    Book *book;
    Car *car;
} ItemData;

// Определение структуры элемента массива
typedef struct {
    ItemType type;
    ItemData data;
} ArrayItem;

// Функция для создания объекта Person
ArrayItem* create_person(char* name, int age) {
    ArrayItem* item = (ArrayItem*)malloc(sizeof(ArrayItem));
    if (item != NULL) {
        item->type = PERSON;
        item->data.person = (Person*)malloc(sizeof(Person));
        if (item->data.person != NULL) {
            item->data.person->name = strdup(name); // копируем строку
            item->data.person->age = age;
        } else {
            free(item);
            item = NULL;
        }
    }
    return item;
}

// Функция для создания объекта Book
ArrayItem* create_book(char* title, char* author, int pages) {
    ArrayItem* item = (ArrayItem*)malloc(sizeof(ArrayItem));
    if (item != NULL) {
        item->type = BOOK;
        item->data.book = (Book*)malloc(sizeof(Book));
        if (item->data.book != NULL) {
            item->data.book->title = strdup(title); // копируем строку
            item->data.book->author = strdup(author); // копируем строку
            item->data.book->pages = pages;
        } else {
            free(item);
            item = NULL;
        }
    }
    return item;
}

// Функция для создания объекта Car
ArrayItem* create_car(char* brand, char* model, int year) {
    ArrayItem* item = (ArrayItem*)malloc(sizeof(ArrayItem));
    if (item != NULL) {
        item->type = CAR;
        item->data.car = (Car*)malloc(sizeof(Car));
        if (item->data.car != NULL) {
            item->data.car->brand = strdup(brand); // копируем строку
            item->data.car->model = strdup(model); // копируем строку
            item->data.car->year = year;
        } else {
            free(item);
            item = NULL;
        }
    }
    return item;
}

// Функция для печати элемента массива
void print_item(const ArrayItem *item) {
    switch (item->type) {
    case PERSON:
        printf("Person - Name: %s, Age: %d\n", item->data.person->name, item->data.person->age);
        break;
    case BOOK:
        printf("Book - Title: %s, Author: %s, Pages: %d\n", item->data.book->title, item->data.book->author, item->data.book->pages);
        break;
    case CAR:
        printf("Car - Brand: %s, Model: %s, Year: %d\n", item->data.car->brand, item->data.car->model, item->data.car->year);
        break;
    default:
        printf("Unknown type\n");
    }
}

// Функция для освобождения памяти объекта Person
void free_person(Person *person) {
    free(person->name);
    free(person);
}

// Функция для освобождения памяти объекта Book
void free_book(Book *book) {
    free(book->title);
    free(book->author);
    free(book);
}

// Функция для освобождения памяти объекта Car
void free_car(Car *car) {
    free(car->brand);
    free(car->model);
    free(car);
}

// Функция для освобождения памяти элемента массива
void free_item(ArrayItem *item) {
    switch (item->type) {
    case PERSON:
        free_person(item->data.person);
        break;
    case BOOK:
        free_book(item->data.book);
        break;
    case CAR:
        free_car(item->data.car);
        break;
    default:
        printf("Unknown type\n");
    }
    free(item);
}

// Функция для удаления элемента из массива
void delete_item(ArrayItem** items, int* size, int index) {
    if (index < 0 || index >= *size) {
        printf("Index out of bounds\n");
        return;
    }
    free_item(items[index]);
    for (int i = index; i < *size - 1; i++) {
        items[i] = items[i + 1];
    }
    items[*size - 1] = NULL;
    (*size)--;
}

// Функции для изменения полей объектов
void update_person_name(ArrayItem *item, char *new_name) {
    if (item->type == PERSON) {
        free(item->data.person->name); // освобождаем старую строку
        item->data.person->name = strdup(new_name); // копируем новую строку
    }
}

void update_person_age(ArrayItem *item, int new_age) {
    if (item->type == PERSON) {
        item->data.person->age = new_age;
    }
}

void update_book_title(ArrayItem *item, char *new_title) {
    if (item->type == BOOK) {
        free(item->data.book->title); // освобождаем старую строку
        item->data.book->title = strdup(new_title); // копируем новую строку
    }
}

void update_book_author(ArrayItem *item, char *new_author) {
    if (item->type == BOOK) {
        free(item->data.book->author); // освобождаем старую строку
        item->data.book->author = strdup(new_author); // копируем новую строку
    }
}

void update_book_pages(ArrayItem *item, int new_pages) {
    if (item->type == BOOK) {
        item->data.book->pages = new_pages;
    }
}

void update_car_brand(ArrayItem *item, char *new_brand) {
    if (item->type == CAR) {
        free(item->data.car->brand); // освобождаем старую строку
        item->data.car->brand = strdup(new_brand); // копируем новую строку
    }
}

void update_car_model(ArrayItem *item, char *new_model) {
    if (item->type == CAR) {
        free(item->data.car->model); // освобождаем старую строку
        item->data.car->model = strdup(new_model); // копируем новую строку
    }
}

void update_car_year(ArrayItem *item, int new_year) {
    if (item->type == CAR) {
        item->data.car->year = new_year;
    }
}

int main() {
    int size = 4;

    // Создание массива указателей на элементы
    ArrayItem* items[size];

    // Добавление объектов в массив
    items[0] = create_person("Alice", 30);
    items[1] = create_person("Bob", 25);
    items[2] = create_book("1984", "George Orwell", 328);
    items[3] = create_car("Toyota", "Camry", 2020);

    // Печать всех элементов массива
    printf("Before update:\n");
    for (int i = 0; i < size; i++) {
        if (items[i] != NULL) {
            print_item(items[i]);
        }
    }

    // Изменение полей
    update_person_name(items[0], "Alicia");
    update_person_age(items[1], 26);
    update_book_title(items[2], "Animal Farm");
    update_car_brand(items[3], "Honda");

    // Печать всех элементов массива после изменений
    printf("\nAfter update:\n");
    for (int i = 0; i < size; i++) {
        if (items[i] != NULL) {
            print_item(items[i]);
        }
    }

    // Освобождение памяти всех элементов массива
    for (int i = 0; i < size; i++) {
        if (items[i] != NULL) {
            free_item(items[i]);
        }
    }

    return 0;
}
