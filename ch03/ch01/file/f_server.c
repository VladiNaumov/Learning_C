/* многопоточного сервера-калькулятора с enum + parser */
...
int parse_input(const char *buffer, Operation *op, double *num1, double *num2) {
    char *token;
    char *input_copy = _strdup(buffer); // Копируем строку для безопасной модификации

    // Удаляем пробелы в начале и конце строки
    char *end;
    while (isspace((unsigned char)*input_copy)) input_copy++;
    if (*input_copy == 0) return 0; // Пустая строка

    // Получаем первый токен (операцию)
    ...
    if (strcmp(token, "PLUS") == 0) {
        ...
        *op = OP_MINUS;
    } else if (strcmp(token, "MULTIPLICATION") == 0) {
        *op = OP_MULTIPLICATION;
    } else if (strcmp(token, "DIVISION") == 0) {
        *op = OP_DIVISION;
    } else {
        *op = OP_INVALID;
    }

    // Получаем второй токен (первое число)
    token = strtok(NULL, "&");
    if (token == NULL) {
        free(input_copy);
        return 0; // Некорректный ввод
    ...
    *num1 = strtod(token, &endptr);
    if (errno != 0 || *endptr != '\0') { // Проверка на ошибки
        free(input_copy);
        return 0; // Некорректный ввод
    }

    // Получаем третий токен (второе число)
    token = strtok(NULL, "&");
    if (token == NULL) {
        free(input_copy);
        return 0; // Некорректный ввод
