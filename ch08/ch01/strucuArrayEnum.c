...
// Создаем массив ArrayItem (в стеке) для хранения объектов
    ...
    for (int i = 0; i < counter; i++) {
        ...
        /*
        Конструкция Person *p = (Person *)array[i].data; используется для приведения указателя общего типа void* к указателю конкретного типа Person*.
        ...
        data — это поле структуры ArrayItem, которое хранит указатель на данные типа void*.
        Поскольку void* может указывать на данные любого типа, это позволяет нам хранить указатели на объекты различных типов в одном массиве.
        (Person *)array[i].data:

        (Person *) — это операция приведения типа (каст), которая преобразует указатель общего типа void* к указателю конкретного типа Person*.
        Мы делаем это, чтобы сказать компилятору, что данные, на которые указывает data, на самом деле имеют тип Person.
        Person *p = (Person *)array[i].data;:

        Здесь мы создаем переменную p типа Person* и инициализируем ее значением (Person *)array[i].data.
        После приведения типа p становится указателем на структуру Person, и мы можем использовать его для доступа к полям структуры Person.
        
        Причина использования приведения типа:
        Поле data в структуре ArrayItem имеет тип void*, что позволяет ему хранить указатель на данные любого типа.
        Чтобы работать с этими данными как с конкретным типом (Person, Book, или Car), нам нужно явно привести void* к нужному типу указателя.

        Безопасность приведения:
        Приведение типа работает корректно только если вы уверены в том, что data действительно указывает на объект нужного типа.
        В нашем примере мы проверяем item->type, чтобы убедиться, что data содержит указатель на Person, перед приведением.

        Заключение
        Конструкция Person *p = (Person *)array[i].data; используется для приведения указателя типа void* к указателю типа Person*, что позволяет работать с данными как с объектами типа Person.
        Это приведение необходимо из-за использования обобщенной структуры ArrayItem, которая может хранить указатели на данные разных типов.
