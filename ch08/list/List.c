...
/*
Все	определения, приведенные в этом	листинге, являются приватными.
Внешней логике,	которая	будет использовать объект list_t, ничего о них неизвестно,
и полагаться она может только на код в заголовочном	файле.
Заметьте, что в	данном листинге	не было	подключено никаких заголовков!
Совпадений сигнатур	функций	с объявлениями в заголовочном файле	достаточно.
Но все же заголовки	рекомендуется подключать, поскольку	это	гарантирует
совместимость между	объявлениями и соответствующими	определениями.
*/